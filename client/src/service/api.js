...
axiosInstance.interceptors.request.use(
    function(config) {
        ...
        // console.log('config id type', config.TYPE.query);
        return config;
    },
    function(error) {
...
for (const [key, value] of Object.entries(SERVICE_URLS)) {
    API[key] = (body, showUploadProgress, showDownloadProgress) => {
        // console.log(body, showUploadProgress, showDownloadProgress)
        return axiosInstance({
            method: value.method,
            url: value.url,
            data: value.method === 'DELETE'? {id: body} : body,
            responseType: value.responseType,
            headers: {
                authorization: getAccessToken(),
                ...(value.multipart && {'Content-Type': 'multipart/form-data'}),
            },
